...
// 技能：三形态共享（解锁即可用），每个技能有开局加卡 + 被动标记（用于引擎强制规则）
data class SkillMeta(
    val id: String,
    val label: String,
    val desc: String,
    val setupExtra: List<String> = emptyList(),
    val passiveFlags: Set<String> = emptySet()   // 写入 GameState.skillFlags，引擎检查
)
...
object Aoi {
    ...
    private fun trap(id: String, name: String, tt: TrapType, tags: List<EffectTag> = emptyList(), text: String = "") =
        Card(id, name, TRAP, trapType = tt, effectTags = tags, text = text)

    // ========== 淘气仙星 ==========
    private val trickstarCards: List<Card> = listOf(
        mon("t_candina", "淘气仙星·坎蒂娜", 4, null, null, 1800, 400, LIGHT, FAIRY, tags = listOf(DRAW_2), text = "通常召唤成功时：从卡组加1张淘气仙星。"),
        mon("t_lycoris", "淘气仙星·里可丽丝", 3, null, null, 1600, 1200, LIGHT, FAIRY, tags = listOf(BURN_500), text = "对方将卡加入手牌时：给对方200伤害。"),
        mon("t_apple", "淘气仙星·林檎", 3, null, null, 1500, 800, LIGHT, FAIRY, tags = listOf(DESTROY_ONE_MONSTER), text = "淘气仙星怪兽被战斗破坏时：破坏1只对方怪兽。"),
        mon("t_cat", "淘气仙星·喵绒", 1, null, null, 700, 300, LIGHT, FAIRY, tags = listOf(BURN_500), text = "对方将卡加入手牌时：给对方200伤害。"),
        mon("t_holly", "淘气仙星·蜀葵天使", null, null, 2, 2000, null, LIGHT, FAIRY, kind = SummonKind.LINK,
            arrows = listOf(BOTTOMLEFT, BOTTOMRIGHT), tags = listOf(DESTROY_ONE_MONSTER),
            text = "连接2：淘气仙星x1。连接召唤成功时：从墓地回收1只淘气仙星。", mats = listOf("淘气仙星怪兽1只")),
        spell("t_stage", "淘气仙星灯光舞台", SpellType.FIELD, tags = listOf(BURN_500),
            text = "对方将卡加入手牌时：给对方200伤害。"),
        trap("t_festival", "淘气仙星·万圣节", TrapType.NORMAL, tags = listOf(BURN_500),
    ...
    private val marincessCards: List<Card> = listOf(
        mon("m_blue", "海晶少女 蓝海", 4, null, null, 1600, 1000, WATER, CYBERSE,
            tags = listOf(REVIVE_ONE), text = "通常召唤成功时：从卡组特殊召唤1只「海晶少女」怪兽。"),
        mon("m_dolphin", "海晶少女 海豚", 4, null, null, 1200, 1400, WATER, CYBERSE,
            tags = listOf(REVIVE_ONE), text = "通常召唤成功时：从墓地回收1只「海晶少女」怪兽。"),
        mon("m_pascal", "海晶少女 帕斯卡", 3, null, null, 800, 1400, WATER, CYBERSE,
            tags = listOf(DESTROY_ONE_MONSTER), text = "通常召唤成功时：破坏对方1只表侧表示怪兽。"),
        mon("m_jelly", "海晶少女 水母", 3, null, null, 1400, 1000, WATER, CYBERSE,
            tags = listOf(BURN_500), text = "对方特殊召唤时：给对方500伤害。"),
        mon("m_tang", "海晶少女 珊瑚海葵", null, null, 2, 2000, null, WATER, CYBERSE, kind = SummonKind.LINK,
            arrows = listOf(LEFT, BOTTOM), tags = listOf(REVIVE_ONE),
        ...
        mon("m_reef", "海晶少女 魔泡大堡垒", null, null, 3, 2300, null, WATER, CYBERSE, kind = SummonKind.LINK,
            arrows = listOf(LEFT, BOTTOM, RIGHT), tags = listOf(DRAW_2),
        ...
        mon("m_heart", "海晶少女 奇迹心", null, null, 4, 2400, null, WATER, CYBERSE, kind = SummonKind.LINK,
            arrows = listOf(TOP, BOTTOM, LEFT, RIGHT), tags = listOf(BUFF_SELF_500),
            text = "连接4：海晶少女x3。每控制1只「海晶少女」：此卡攻击力+500。不会被对方卡的效果破坏。", mats = listOf("海晶少女怪兽3只")),
        trap("m_wave", "海晶少女波动", TrapType.NORMAL, tags = listOf(DESTROY_ONE_MONSTER),
            text = "对方怪兽发动效果时：使其效果无效并破坏。"),
        spell("m_bounce", "海晶少女大波浪", SpellType.NORMAL, tags = listOf(DRAW_2),
            text = "从卡组加2只「海晶少女」怪兽上手，然后1张手牌回卡组。")
    )
